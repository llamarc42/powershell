...
Describe 'Get-AiContextContent' {
    ...
    Context 'Without headers' {
        It 'combines all file contents into one string' {
            $result = Get-AiContextContent -Files @($fileInfo1, $fileInfo2) -RootPath $rootPath
            $result | Should -Match 'Introduction content'
            $result | Should -Match 'Guide content'
        }

        It 'returns a non-empty string' {
            $result = Get-AiContextContent -Files @($fileInfo1) -RootPath $rootPath
    ...
    Context 'With -IncludeHeaders' {
        It 'adds BEGIN FILE and END FILE markers' {
            $result = Get-AiContextContent -Files @($fileInfo1) -RootPath $rootPath -IncludeHeaders
        ...
        It 'includes the nested relative path in the header' {
            $result = Get-AiContextContent -Files @($fileInfo2) -RootPath $rootPath -IncludeHeaders
    ...
    Context 'When a file is outside the root path' {
        It 'falls back to the file name in the header' {
            $result = Get-AiContextContent -Files @($fileInfoOut) -RootPath $rootPath -IncludeHeaders

BeforeAll {
    ...
    $session = Get-OllamaProjectSession -Path $sessionFolder
}
...
Describe 'Get-OllamaProjectSessionMessage' {
    Context 'Reading all messages' {
        It 'returns three messages' {
            $messages = Get-OllamaProjectSessionMessage -Session $session
            $messages.Count | Should -Be 3
        }

        It 'returns objects with PSTypeName Ollama.ProjectSessionMessage' {
            $messages = Get-OllamaProjectSessionMessage -Session $session
            foreach ($m in $messages) {
                $m.PSObject.TypeNames[0] | Should -Be 'Ollama.ProjectSessionMessage'
            }
        }
    ...
    Context 'With -Tail parameter' {
        It 'returns only the last N messages' {
            $messages = Get-OllamaProjectSessionMessage -Session $session -Tail 2
    ...
    Context 'With -Raw parameter' {
        It 'returns raw deserialized objects without PSTypeName' {
            $messages = Get-OllamaProjectSessionMessage -Session $session -Raw
            foreach ($m in $messages) {
                $m.PSObject.TypeNames[0] | Should -Not -Be 'Ollama.ProjectSessionMessage'
            }
        }
    ...
    Context 'By -Path parameter set' {
        It 'returns messages when given a session folder path' {
            $messages = Get-OllamaProjectSessionMessage -Path $sessionFolder
            $messages.Count | Should -Be 3
        }
    }
    ...
    Context 'When the messages file does not exist' {
        BeforeAll {
            ...
            $badSession = Get-OllamaProjectSession -Path $badFolder
        }

        It 'throws an error' {
            { Get-OllamaProjectSessionMessage -Session $badSession } | Should -Throw
        }
    }
    ...
    Context 'When the messages file is empty' {
        BeforeAll {
            ...
            $emptySession = Get-OllamaProjectSession -Path $emptyFolder
        }

        It 'returns an empty array' {
            $messages = Get-OllamaProjectSessionMessage -Session $emptySession

...
Describe 'Get-AiProjectContext' {
    Context 'Return object shape' {
        It 'returns an object with the expected properties' {
            $result = Get-AiProjectContext -ProjectFolder $projectDir
    ...
    Context 'With -IncludeFileHeaders' {
        It 'adds file markers to the combined content' {
            $result = Get-AiProjectContext -ProjectFolder $projectDir -IncludeFileHeaders
    ...
    Context 'Extension filtering' {
        ...
        It 'includes .txt files when requested' {
            $result = Get-AiProjectContext -ProjectFolder $projectDir -IncludeExtensions '.md', '.txt'
            $result.ProjectFiles | Should -Contain (Join-Path $projectDir 'readme.txt')
        }

        It 'excludes .txt files with default extensions' {
            $result = Get-AiProjectContext -ProjectFolder $projectDir -IncludeExtensions '.md'

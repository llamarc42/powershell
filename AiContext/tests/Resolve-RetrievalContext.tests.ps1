...
Describe 'Resolve-RetrievalContext' {
    Context 'Return object shape' {
        It 'returns an object with PSTypeName Llamarc42.RetrievalContext' {
            $result = Resolve-RetrievalContext -ProjectFolder $projectDir -Policy $fakePolicy
        ...
        It 'echoes the resolved intent' {
            $result = Resolve-RetrievalContext -Intent planning -ProjectFolder $projectDir -Policy $fakePolicy
    ...
    Context 'Invalid intent in policy' {
        It 'throws when the policy has no strategy for the given intent' {
            ...
            { Resolve-RetrievalContext -Intent general -ProjectFolder $projectDir -Policy $noStrategyPolicy } |
                Should -Throw
        }
    }
}

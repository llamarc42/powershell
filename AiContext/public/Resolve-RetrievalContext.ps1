...
.EXAMPLE
Resolve-RetrievalContext -Intent planning -ProjectFolder .

.EXAMPLE
$policy = Get-RetrievalPolicy
Resolve-RetrievalContext -Intent review -Policy $policy

.OUTPUTS
Llamarc42.RetrievalContext
#>
function Resolve-RetrievalContext {
    [CmdletBinding()]
    param(
        [Parameter()]
    ...
    if (-not $Policy) {
        $Policy = Get-RetrievalPolicy -PolicyPath $PolicyPath -ProjectFolder $paths.ProjectFolder
    }

...
Describe 'Add-Llamarc42ProjectSessionMessage' {
    BeforeEach {
    ...
    Context 'Return value' {
        It 'returns an Llamarc42.ProjectSessionMessage object' {
            $result = Add-Llamarc42ProjectSessionMessage -Session $session -Role user -Content 'Hello'
            $result.PSObject.TypeNames[0] | Should -Be 'Llamarc42.ProjectSessionMessage'
        }

        It 'returns the correct Role' {
            $result = Add-Llamarc42ProjectSessionMessage -Session $session -Role assistant -Content 'Hi'
            $result.Role | Should -Be 'assistant'
        }

        It 'returns the correct Content' {
            $result = Add-Llamarc42ProjectSessionMessage -Session $session -Role user -Content 'Test message'
    ...
    Context 'File persistence' {
        It 'appends a line to messages.jsonl' {
            Add-Llamarc42ProjectSessionMessage -Session $session -Role user -Content 'First' | Out-Null
        ...
        It 'appends multiple messages in order' {
            Add-Llamarc42ProjectSessionMessage -Session $session -Role user      -Content 'First'  | Out-Null
            Add-Llamarc42ProjectSessionMessage -Session $session -Role assistant -Content 'Second' | Out-Null
            $lines = @(Get-Content -LiteralPath $session.MessagesFile | Where-Object { $_ -ne '' })
        ...
        It 'writes valid JSON for each line' {
            Add-Llamarc42ProjectSessionMessage -Session $session -Role system -Content 'System init' | Out-Null
    ...
    Context 'Metadata update' {
        It 'increments the MessageCount in the session metadata' {
            Add-Llamarc42ProjectSessionMessage -Session $session -Role user -Content 'Hello' | Out-Null
            $updated = Get-Llamarc42ProjectSession -Path $session.SessionFolder
    ...
    Context 'All supported roles' {
        It 'accepts role "user"' {
            { Add-Llamarc42ProjectSessionMessage -Session $session -Role user -Content 'x' } |
                Should -Not -Throw
        }

        It 'accepts role "assistant"' {
            { Add-Llamarc42ProjectSessionMessage -Session $session -Role assistant -Content 'x' } |
                Should -Not -Throw
        }

        It 'accepts role "system"' {
            { Add-Llamarc42ProjectSessionMessage -Session $session -Role system -Content 'x' } |
                Should -Not -Throw
        }
    }
    ...
    Context 'Validation errors' {
        It 'throws when Content is empty' {
            { Add-Llamarc42ProjectSessionMessage -Session $session -Role user -Content '' } |
                Should -Throw
        }
    ...
    Context 'ByPath parameter set' {
        It 'appends a message when given a session folder path' {
            Add-Llamarc42ProjectSessionMessage -Path $session.SessionFolder -Role user -Content 'Via path' | Out-Null
            $lines = @(Get-Content -LiteralPath $session.MessagesFile | Where-Object { $_ -ne '' })
            $lines.Count | Should -Be 1
        }
    }

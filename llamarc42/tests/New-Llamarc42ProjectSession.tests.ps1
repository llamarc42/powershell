...
Describe 'New-Llamarc42ProjectSession' {
    Context 'Default parameters' {
        BeforeAll {
            $session = New-Llamarc42ProjectSession -ProjectFolder $projectDir
        }

        It 'returns an Llamarc42.ProjectSession object' {
            $session.PSObject.TypeNames[0] | Should -Be 'Llamarc42.ProjectSession'
        }
    ...
    Context 'Custom name and title' {
        It 'slugifies the Name into the session id' {
            $s = New-Llamarc42ProjectSession -Name 'API Review' -ProjectFolder $projectDir
            $s.Name | Should -Be 'api-review'
        }

        It 'uses the provided Title' {
            $s = New-Llamarc42ProjectSession -Name 'my-session' -Title 'My Custom Title' -ProjectFolder $projectDir
            $s.Title | Should -Be 'My Custom Title'
        }

        It 'auto-generates a Title from the slug when Title is omitted' {
            $s = New-Llamarc42ProjectSession -Name 'hello-world' -ProjectFolder $projectDir
    ...
    Context 'Custom model' {
        It 'stores the specified model' {
            $s = New-Llamarc42ProjectSession -Model 'llama3:8b' -ProjectFolder $projectDir
    ...
    Context 'Tags and extensions' {
        It 'stores the provided tags' {
            $s = New-Llamarc42ProjectSession -Tags @('ci', 'review') -ProjectFolder $projectDir
            $s.Tags | Should -Contain 'ci'
            $s.Tags | Should -Contain 'review'
        }

        It 'stores the provided ArtifactExtensions' {
            $s = New-Llamarc42ProjectSession -Name 'test-extensions' -ArtifactExtensions @('.md', '.txt') -ProjectFolder $projectDir
    ...
    Context 'When the session folder already exists' {
        It 'throws an error' {
            ...
            $s = New-Llamarc42ProjectSession -Name 'duplicate-test' -ProjectFolder $projectDir
            ...
            { New-Llamarc42ProjectSession -Name 'duplicate-test' -ProjectFolder $projectDir } |
                Should -Throw
        }
    }
}

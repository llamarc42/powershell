...
Describe 'Select-Llamarc42ProjectSession' {
    Context 'When the user selects a valid session' {
        BeforeAll {
            Mock Read-Host -ModuleName llamarc42 { '1' }
        }

        It 'returns an Llamarc42.ProjectSession object' {
            $result = Select-Llamarc42ProjectSession -ProjectFolder $projectDir
    ...
    Context 'When -AllowNew is used and the user picks the new session option' {
        BeforeAll {
            ...
            Mock Read-Host -ModuleName llamarc42 { '3' }
        }

        It 'returns $null to signal a new session should be created' {
            $result = Select-Llamarc42ProjectSession -ProjectFolder $projectDir -AllowNew
    ...
    Context 'When there are no sessions and -AllowNew is not set' {
        ...
        It 'throws an error' {
            { Select-Llamarc42ProjectSession -ProjectFolder $emptyProj } | Should -Throw
        }
    }
    ...
    Context 'When there are no sessions and -AllowNew is set' {
        ...
        It 'returns $null without prompting' {
            $result = Select-Llamarc42ProjectSession -ProjectFolder $emptyProj2 -AllowNew
            $result | Should -BeNullOrEmpty
        }
    }
}

...
Describe 'Get-Llamarc42ProjectSessionList' {
    Context 'Listing all sessions' {
        It 'returns three sessions' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir
            $list.Count | Should -Be 3
        }

        It 'returns sessions with Llamarc42.ProjectSessionInfo PSTypeName' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir
            foreach ($item in $list) {
                $item.PSObject.TypeNames[0] | Should -Be 'Llamarc42.ProjectSessionInfo'
            }
        }
    ...
    Context 'Name filter' {
        It 'returns only sessions matching the Name by Name property' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir -Name 'beta'
            $list.Count | Should -Be 1
            $list[0].Name | Should -Be 'beta'
        }

        It 'returns only sessions matching the Name by Title property' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir -Name 'Alpha'
            $list.Count | Should -Be 1
            $list[0].Title | Should -Be 'Alpha Session'
        }

        It 'returns empty array when no session matches' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir -Name 'no-match'
    ...
    Context '-First limit' {
        It 'returns only the first N sessions' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir -First 2
            $list.Count | Should -Be 2
        }

        It 'returns the newest sessions when First is applied' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $projectDir -First 1
            $list[0].Id | Should -Be '2026-03-01_120000-gamma'
        }
    }
    ...
    Context 'When no .sessions folder exists' {
        ...
        It 'returns an empty array' {
            $list = Get-Llamarc42ProjectSessionList -ProjectFolder $emptyProj

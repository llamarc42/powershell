...
Describe 'Resume-Llamarc42ProjectSession' {
    Context 'No -Name specified' {
        It 'returns the most recent session' {
            $result = Resume-Llamarc42ProjectSession -ProjectFolder $projectDir
    ...
    Context 'Matching by Name' {
        It 'returns the session matching the name' {
            $result = Resume-Llamarc42ProjectSession -Name 'alpha' -ProjectFolder $projectDir
    ...
    Context 'Matching by Title' {
        It 'returns the session matching a partial title' {
            $result = Resume-Llamarc42ProjectSession -Name 'Beta' -ProjectFolder $projectDir
    ...
    Context 'Matching by Id' {
        It 'returns the session matching a partial id' {
            $result = Resume-Llamarc42ProjectSession -Name '2026-03-01' -ProjectFolder $projectDir
            $result.Id | Should -Be '2026-03-01_120000-review'
        }
    }
    ...
    Context 'No sessions exist' {
        ...
        It 'throws an error when no sessions are found' {
            { Resume-Llamarc42ProjectSession -ProjectFolder $emptyProj } | Should -Throw
        }
    }

    Context 'Name matches no session' {
        It 'throws an error' {
            { Resume-Llamarc42ProjectSession -Name 'no-such-session' -ProjectFolder $projectDir } |
                Should -Throw
        }
    }
    ...
    Context 'Name matches multiple sessions' {
        It 'throws an error listing the ambiguous matches' {
            ...
            { Resume-Llamarc42ProjectSession -Name 'Session' -ProjectFolder $projectDir } |
                Should -Throw
        }
    }
}

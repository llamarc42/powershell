...
.EXAMPLE
Get-Llamarc42ProjectContext -ProjectFolder . -IncludeExtensions '.md', '.txt'

.OUTPUTS
System.Management.Automation.PSCustomObject
#>
function Get-Llamarc42ProjectContext {
    [CmdletBinding()]
    param(
        [Parameter()]
    ...
    $globalContent = Get-Llamarc42Content `
        -Files $globalFiles `
        -RootPath $paths.GlobalFolder `
        -IncludeHeaders:$IncludeFileHeaders

    $projectContent = Get-Llamarc42Content `
        -Files $projectFiles `

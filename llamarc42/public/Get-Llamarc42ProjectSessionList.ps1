...
.EXAMPLE
Get-Llamarc42ProjectSessionList -ProjectFolder . -First 10

.OUTPUTS
Llamarc42.ProjectSessionInfo
#>
function Get-Llamarc42ProjectSessionList {
    [CmdletBinding()]
    param(
        [Parameter()]
    ...
    $sessions = foreach ($folder in $sessionFolders) {
        ...
        try {
            ...
            [pscustomobject]@{
                PSTypeName    = 'Llamarc42.ProjectSessionInfo'
                Id            = $session.Id
                Name          = $session.Name
                Title         = $session.Title

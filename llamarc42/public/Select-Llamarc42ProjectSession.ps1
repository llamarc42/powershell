...
.EXAMPLE
Select-Llamarc42ProjectSession -ProjectFolder . -AllowNew

.OUTPUTS
Llamarc42.ProjectSession
#>
function Select-Llamarc42ProjectSession {
    [CmdletBinding()]
    param(
        [Parameter()]
    ...
    while ($true) {
        ...
        if ($selectionNumber -ge 1 -and $selectionNumber -le $sessions.Count) {
            return Get-Llamarc42ProjectSession -Path $sessions[$selectionNumber - 1].SessionFolder
        }
